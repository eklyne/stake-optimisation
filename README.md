...
`current_hands` on a stake records the table time really spent there. It is only
ever read as a **ratio** across stakes — enough to rebuild the mix as whole tables
— so the absolute figures and the period they cover do not matter, as long as they
are consistent across the ladder.

```toml
current_hands = 23082                              # one period
current_hands = { July = 23082, August = 15183 }   # several, oldest first
```

Name several and the deck carries them all: each gets its own circle on both
frontier charts and its own opening row in every allocation table, so the mix is
seen **moving** against a frontier that has not. Written oldest first, and the
**last one is now** — it is the mix the simulations, the step-up slide and the
dominance argument are about; the earlier ones ride along as history. Now is the
solid blue circle, earlier periods the same circle hollow, so the ordering
survives a greyscale print. A stake missing from a period played none of it.

## Refreshing the inputs

`config.toml` is the only file this pipeline reads. Every number in it about how
you actually run — win rate, standard deviation, rake, hand counts, the periods —
comes out of the hand histories in the sibling **nemesis-mvp** repo:

```
cd ..\nemesis-mvp
.venv\Scripts\python.exe -X utf8 55_export_stake_inputs.py --to ../stake-optimisation
```

That writes `stake_optimisation_inputs.csv` here (one row per period × stake, plus
a `LIFETIME` row) and **prints the six `[[stake]]` blocks ready to paste**. Paste
the numeric lines over the ones in `config.toml`, leave the prose alone, and run
`run.bat`. `shotopt/config.py` validates strictly, so a bad paste exits 2 with a
named error rather than shipping quietly.

Three things worth knowing about it:

- **It checks itself.** Before exporting anything it re-runs its own query on the
  iPoker-only basis and asserts it reproduces the hero rows of nemesis's
  `player_stats_by_stake.csv` on hands, bb/100 and rake/100. `player_stats.sql`
  is the oracle; the exporter is held to it and fails loudly rather than drifting.
  A number here can still be stale — it can no longer be mistyped.
- **The periods are automatic.** It emits the newest two calendar months in the
  database, oldest first, named. Next month that is August + September with no
  edit. `--periods 3` widens it.
- **It does not overwrite your judgement.** Which stakes get a measured win rate
  and which get an assumed one lives in the exporter's `ASSUMED_WINRATE` table,
  and the standard-deviation floor (`MIN_HANDS_FOR_MEASURED_SD`) is applied there
  too — so the reasoning written into the comments below each stake keeps being
  applied on every refresh instead of being re-argued from scratch. Change the
  policy there, not by editing the printed output.

`stake_data.csv`, `july_stake_dist.csv` and `aug_stake_dist.csv` are the
PokerTracker exports this used to be transcribed from by hand. They are kept as
the historical record and are no longer the source of anything.

The one figure that is **not** refreshed this way is
`hands_per_hour_per_table` — hand histories carry no session clock, so it stays a
PokerTracker number. It only scales every €/hour uniformly and cannot move the
ranking or the chosen mix.
...
| `sims.pptx` | the companion deck: twenty single simulated lifetimes each for the optimal and the played mix, one per slide on shared axes. Each lifetime is drawn SPLIT - what the cards paid, the rakeback on the same volume, and the total of the two - and the deepest fall is measured on the table line, with the rakeback earned over that same stretch, and therefore the smaller fall in the bankroll, quoted beside it |
...
```
config.toml          the four inputs — THE ONLY FILE THE PIPELINE READS
stake_optimisation_inputs.csv
                     generated by nemesis-mvp/55_export_stake_inputs.py; the
                     source config.toml's stake numbers are pasted from, and
                     read by a human, not by the code
stake_data.csv, july_stake_dist.csv, aug_stake_dist.csv
                     the superseded PokerTracker exports, kept as history
shotopt/
